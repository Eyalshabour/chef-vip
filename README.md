...
`render.yaml` is a Blueprint: it creates the web service and the Postgres
together, runs the migration and the seed on every build, and health-checks
`/healthz`.

**Set `BOOTSTRAP_CODE` before the first deploy.** Every four-digit code is
set by management from inside the app — and a fresh database has nobody
signed in to set one, so without this the deployment is a locked door. The
seed uses it once, for the director, and only while his code is unset: a
redeploy never overwrites a code already in place. Change it from
**Management → Access** once you are in.

| Variable | |
|---|---|
| `BOOTSTRAP_CODE` | your own four digits, for the first sign-in. Required once. |
| `DATABASE_URL` | wired to the database by the Blueprint. |
| `SESSION_SECRET` | generated by the Blueprint. |
| `BOOTSTRAP_FORCE` | the way back in. `1` alongside a new `BOOTSTRAP_CODE` replaces the director's code on the next deploy and clears any lockout. Remove it immediately afterwards. |
| `MELBA_API_KEY` | optional. Without it the Melba panel answers 503 and the rest of the board is unaffected. |
| `MELBA_API_BASE` | **unverified** — `https://api.melba.io` is a guess. Confirm it before trusting any figure it returns. |

### Locked out

The first-code mechanism deliberately refuses to overwrite a code that already
exists — otherwise anyone who ever read the environment could let themselves
back in. So if the director's code is lost, or the account is locked:

1. Set `BOOTSTRAP_CODE` to new digits **and** add `BOOTSTRAP_FORCE=1`.
2. Deploy. The log says `director's code RESET from BOOTSTRAP_CODE`.
3. Sign in, change the code from **Management → Access**.
4. **Remove `BOOTSTRAP_FORCE`.** While it is set, every deploy resets the code.

Everyone else's code is reset from Management by anyone in management, which
is the normal path and needs no deploy.
